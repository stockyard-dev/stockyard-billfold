...
.filters{display:flex;gap:.3rem;margin-bottom:1rem}
.fbtn{font-size:.6rem;padding:.2rem .5rem;border:1px solid var(--bg3);background:var(--bg);color:var(--cm);cursor:pointer}.fbtn:hover{border-color:var(--leather)}.fbtn.active{border-color:var(--rust);color:var(--rust)}
.inv{background:var(--bg2);border:1px solid var(--bg3);padding:.8rem 1rem;margin-bottom:.4rem}
.inv-top{display:flex;justify-content:space-between;align-items:center}
.inv-name{font-size:.82rem;color:var(--cream)}
.inv-amt{font-size:.9rem;font-weight:bold}
.inv-meta{font-size:.6rem;color:var(--cm);margin-top:.2rem;display:flex;gap:.8rem}
.badge-draft{color:var(--cm);border:1px solid var(--bg3);font-size:.5rem;padding:.1rem .3rem;text-transform:uppercase}
.badge-sent{color:var(--orange);border:1px solid #d4843a44;font-size:.5rem;padding:.1rem .3rem;text-transform:uppercase}
.badge-paid{color:var(--green);border:1px solid #4a9e5c44;font-size:.5rem;padding:.1rem .3rem;text-transform:uppercase}
.badge-overdue{color:var(--red);border:1px solid #c9444444;font-size:.5rem;padding:.1rem .3rem;text-transform:uppercase}
...
<script>
const A='/api';let invoices=[],filter='';
async function load(){const[i,s]=await Promise.all([fetch(A+'/invoices').then(r=>r.json()),fetch(A+'/stats').then(r=>r.json())]);
invoices=i.invoices||[];
const total=invoices.reduce((s,i)=>s+i.amount,0);
const paid=invoices.filter(i=>i.status==='paid').reduce((s,i)=>s+i.amount,0);
const outstanding=invoices.filter(i=>i.status!=='paid'&&i.status!=='draft').reduce((s,i)=>s+i.amount,0);
document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+invoices.length+'</div><div class="st-l">Invoices</div></div><div class="st"><div class="st-v">$'+fmt(total/100)+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v" style="color:var(--green)">$'+fmt(paid/100)+'</div><div class="st-l">Paid</div></div><div class="st"><div class="st-v" style="color:var(--orange)">$'+fmt(outstanding/100)+'</div><div class="st-l">Outstanding</div></div>';
let fh='';['','draft','sent','paid','overdue'].forEach(s=>{fh+='<button class="fbtn'+(filter===s?' active':'')+'" onclick="setFilter(\''+s+'\')">'+(!s?'All':s)+'</button>';});
document.getElementById('filters').innerHTML=fh;render();}
function setFilter(f){filter=f;render();}
function render(){let filtered=filter?invoices.filter(i=>i.status===filter):invoices;
...
h+='<div class="inv"><div class="inv-top"><div class="inv-name">'+esc(i.name)+'</div><div style="display:flex;gap:.4rem;align-items:center"><span class="inv-amt'+(i.status==='paid'?' style="color:var(--green)"':'')+'">$'+(i.amount/100).toFixed(2)+'</span><span class="badge-'+i.status+'">'+i.status+'</span></div></div>';
h+='<div class="inv-meta">';if(i.due_date)h+='<span>Due: '+i.due_date+'</span>';if(i.paid_at)h+='<span>Paid: '+ft(i.paid_at)+'</span>';h+='<span>Created '+ft(i.created_at)+'</span>';
h+='<span style="display:flex;gap:.3rem">';
if(i.status==='draft')h+='<button class="btn" onclick="setStatus(\''+i.id+'\',\'sent\')" style="font-size:.5rem">Mark Sent</button>';
if(i.status==='sent')h+='<button class="btn" onclick="setStatus(\''+i.id+'\',\'paid\')" style="font-size:.5rem;color:var(--green)">Mark Paid</button>';
h+='<button class="btn" onclick="del(\''+i.id+'\')" style="font-size:.5rem;color:var(--cm)">✕</button></span>';
h+='</div></div>';});
